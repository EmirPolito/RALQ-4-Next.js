...
const OrbButtonVariants = cva(
  "group relative inline-flex items-center whitespace-nowrap font-bold transition-all duration-200 focus-visible:outline-hidden focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 overflow-hidden rounded-full border border-[var(--titulos)]/30 hover:border-[var(--titulos)] data-[touched=true]:border-[var(--titulos)] bg-transparent cursor-pointer",
  {
    variants: {
      size: {
        default:
  ...
  ({ className, size, children, Icon, icon, dotClassName, ...props }, ref) => {
    ...
    return (
      ...
      >
        <span
          className={cn(
            "relative shrink-0 rounded-full flex items-center justify-center bg-[var(--foreground)] transition-all duration-200 w-2.5 h-2.5",
            dotExpanded,
            dotClassName,
          )}
        >
          <span className="text-[var(--background)] flex items-center justify-center opacity-0 scale-50 transition-all duration-200 group-hover:opacity-100 group-hover:scale-100 group-data-[touched=true]:opacity-100 group-data-[touched=true]:scale-100">
            {resolvedIcon}
          </span>
        </span>
        <span className="relative z-10 tracking-wide text-[var(--foreground)] transition-transform duration-200 group-hover:translate-x-1.25 group-data-[touched=true]:translate-x-1.25">
          {children}
        </span>
      </button>
    );
