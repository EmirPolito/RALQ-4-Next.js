    ...
    <div className="flex flex-col gap-4">
              ...
              {(["3D", "AR", "360"] as const).map((mode) => (
                <button
                  ...
                  className={cn(
                    ...
                    viewMode === mode ? "bg-cyan-600 text-white shadow-md" : "text-slate-500 hover:text-slate-700"
                  )}
                >
                  {mode}
                </button>
      ...
      <div className="relative h-[400px] rounded-3xl overflow-hidden shadow-2xl border border-slate-200 group">
          ...
          <div className="bg-white/10 backdrop-blur-md p-3 rounded-2xl shadow-xl border border-white/10 max-w-[220px]">
            <div className="flex gap-2">
              ...
              <p className="text-[10px] text-white/80 leading-tight">
                Vista molecular: observa la estructura atómica y los enlaces químicos de la {activeItem.name.toLowerCase()}.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="absolute bottom-6 left-6 right-6 flex justify-between items-center z-20">
          <div className="flex gap-2">
            <ActionButton icon={<RotateCcw className="w-4 h-4" />} label="Reset" isCyan />
            <ActionButton icon={<Maximize className="w-4 h-4" />} label="Expandir" isCyan />
          </div>
          ...
          <div className="flex gap-2 text-white/50 bg-black/20 backdrop-blur-md p-1 rounded-xl border border-white/5">
            ...
            <button className="px-3 py-1 text-[10px] font-bold hover:text-white transition-colors">PDB</button>
            <button className="px-3 py-1 text-[10px] font-bold hover:text-white transition-colors">XYZ</button>
          </div>
        </div>
      </div>
    </div>
...
function ActionButton({ icon, label, secondary, isCyan }: { icon: React.ReactNode; label: string; secondary?: boolean; isCyan?: boolean }) {
  return (
    <motion.button
