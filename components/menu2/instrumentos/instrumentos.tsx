...
export default function InstrumentosPage() {
  ...
  const activeItem = instrumentsData.find(s => s.id === activeSpeciesId) || instrumentsData[0];
      ...
      <header className="flex items-center justify-between px-6 py-2 bg-white/70 backdrop-blur-md rounded-[32px] border border-white/50 shadow-sm">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center border border-blue-100 shadow-inner">
            <div className="w-6 h-6 rounded-full bg-gradient-to-tr from-blue-500 to-cyan-400 flex items-center justify-center text-white">
              <Compass className="w-3.5 h-3.5" />
            </div>
          </div>
          <div className="flex flex-col">
            <h1 className="text-lg font-black text-slate-800 leading-none tracking-tight">
              RALQ Instrumentos Studio
            </h1>
            <span className="text-[10px] font-medium text-blue-400 tracking-wide uppercase">
              Catálogo de Precisión y Control
            </span>
          </div>
        </div>
